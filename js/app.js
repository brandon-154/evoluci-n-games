...
// Compatibilidad con código existente que usa GENRE_LIST
const GENRE_LIST = [...GENRES_DEF, ...TAGS_DEF];

// Base de datos clasificada de los 305 juegos del catálogo
...
// Devuelve array con [género, ...etiquetas] del juego
// Prioridad: 1) Firebase manual, 2) GAME_DB, 3) ['Acción'] como fallback
function getGameTags(name) {
  if (GENRES[name] && Array.isArray(GENRES[name]) && GENRES[name].length) return GENRES[name];
  // Buscar en GAME_DB (exacto o case-insensitive)
  if (GAME_DB[name]) return GAME_DB[name];
  const lower = name.toLowerCase();
  for (const [k, v] of Object.entries(GAME_DB)) {
    if (k.toLowerCase() === lower) return v;
  }
  return ['Acción']; // fallback: siempre tiene al menos un género
}
function getGameGenre(name) { return getGameTags(name)[0] || 'Acción'; }
function getGameETags(name) { return getGameTags(name).slice(1); }
...
function openAddGame() {
  ...
  const _gs = document.querySelector('#ag-genre-sel');
  if (_gs) _gs.value = '';
...
function editGame(con, disk, name) {
  ...
  const _tags = getGameTags(name);
  const _genre = _tags[0] || '';
  const _etags = _tags.slice(1);
  const genSel = document.querySelector('#ag-genre-sel');
  if (genSel) genSel.value = _genre;
  document.querySelectorAll('#ag-tag-checks input').forEach(cb => {
    cb.checked = _etags.includes(cb.value);
  });
...
// activeFilters = Set of active genre/tag strings
...
function renderClientFilterBar(all) {
  ...
  // Collect present genres and tags
  const presentGenres = new Set(), presentTags = new Set();
  all.forEach(g => {
    const t = getGameTags(g.name);
    if (t[0]) presentGenres.add(t[0]);
    t.slice(1).forEach(tag => presentTags.add(tag));
  });

  const activeG = GENRES_DEF.filter(g => presentGenres.has(g));
  const activeT = TAGS_DEF.filter(t => presentTags.has(t));
  if (!activeG.length && !activeT.length) { wrap.style.display = 'none'; return; }

  let html = '';
  if (activeFilters.size > 0) {
    html += `<div class="cf-clear" onclick="clearAllFilters()"><i class="fas fa-times"></i> Limpiar filtros</div>`;
  }
  if (activeG.length) {
    html += `<span class="cf-sep">Género</span>`;
    html += activeG.map(g => {
      const on = activeFilters.has(g);
      return `<div class="ctb cf-chip ${on ? 'on' : ''}" onclick="toggleFilter('${escAttr(g)}')">${g}</div>`;
    }).join('');
  }
  if (activeT.length) {
    html += `<span class="cf-sep">Etiquetas</span>`;
    html += activeT.map(t => {
      const on = activeFilters.has(t);
      return `<div class="ctb cf-chip cf-tag ${on ? 'on' : ''}" onclick="toggleFilter('${escAttr(t)}')">${t}</div>`;
    }).join('');
  }
...
Object.assign(window, {
  ...
  // Client catalog
  renderClientConsoles, renderClientGames, setClientGenre, toggleFilter, clearAllFilters,
  // Print
